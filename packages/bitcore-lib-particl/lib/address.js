...
function Address(data, network, type) {
  ...
  if (type && (
    ...
    && type !== Address.PayToPublicKeyHash256
    && type !== Address.PayToScriptHash256
    && type !== Address.PayToWitnessPublicKeyHash
    && type !== Address.PayToWitnessScriptHash)) {
    throw new TypeError('Third argument must be "pubkeyhash", "scripthash", "pubkeyhash256", "scripthash256", "witnesspubkeyhash" or "witnessscripthash".');
  }
 ...
 * @param {string} data.type - either 'pubkeyhash', 'scripthash', 'pubkeyhash256', 'scripthash256', 'witnesspubkeyhash', or 'witnessscripthash'
 ...
 * @param {string=} type - The type: 'pubkeyhash', 'scripthash', 'pubkeyhash256', 'scripthash256', 'witnesspubkeyhash', or 'witnessscripthash'
...
Address._transformBuffer = function(buffer, network, type) {
  ...
  if (buffer.length > 21) {
    try {
      ...
      if (bufferVersion.type === Address.PayToWitnessPublicKeyHash || bufferVersion.type === Address.PayToWitnessScriptHash) {
        throw e;
      } else {
