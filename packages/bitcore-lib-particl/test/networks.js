...
describe('Networks', function() {
  ...
  var constants = ['name', 'alias', 'pubkeyhash', 'scripthash', 'pubkeyhash256', 'scripthash256', 'xpubkey', 'xprivkey'];
  ...
  it('tests only for the specified key', function() {
    expect(networks.get(0x76, 'pubkeyhash')).to.equal(networks.testnet);
    expect(networks.get(0x76, 'privatekey')).to.equal(undefined);
  });

  it('can test for multiple keys', function() {
    expect(networks.get(0x76, ['pubkeyhash', 'scripthash'])).to.equal(networks.testnet);
    expect(networks.get(0x7a, ['pubkeyhash', 'scripthash'])).to.equal(networks.testnet);
    expect(networks.get(0x76, ['privatekey', 'port'])).to.equal(undefined);
  });
