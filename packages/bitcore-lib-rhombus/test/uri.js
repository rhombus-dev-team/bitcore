...
describe('URI', function() {
  ...
  it('parses uri strings correctly (test vector)', function() {
    ...
    uri = URI.parse('rhombus:PYHrwJA7DimhfYtzVKdZvExJkxJTHFVPok');
    uri.address.should.equal('PYHrwJA7DimhfYtzVKdZvExJkxJTHFVPok');
    expect(uri.amount).to.be.equal(undefined);
    expect(uri.otherParam).to.be.equal(undefined);

    uri = URI.parse('rhombus:PYHrwJA7DimhfYtzVKdZvExJkxJTHFVPok?amount=123.22');
    uri.address.should.equal('PYHrwJA7DimhfYtzVKdZvExJkxJTHFVPok');
  ...
  it('URIs can be validated statically (test vector)', function() {
    URI.isValid('rhombus:PYHrwJA7DimhfYtzVKdZvExJkxJTHFVPok').should.equal(true);
    URI.isValid('rhombus:paFS5X4XuSY9S2dWSe8XKAhPjShHm47yrb').should.equal(true);

    URI.isValid('rhombus:PYHrwJA7DimhfYtzVKdZvExJkxJTHFVPok?amount=1.2')
                .should.equal(true);
    URI.isValid('rhombus:PYHrwJA7DimhfYtzVKdZvExJkxJTHFVPok?amount=1.2&other=param')
                .should.equal(true);
    URI.isValid('rhombus:PYHrwJA7DimhfYtzVKdZvExJkxJTHFVPok?amount=1.2&req-other=param',
                ['req-other']).should.equal(true);
    URI.isValid('rhombus:pkMUkHYKJYTZw96SE1VJZCHiRJoWwS5NtJ?amount=0.1&' +
                'r=https%3A%2F%2Ftest.bitpay.com%2Fi%2F6DKgf8cnJC388irbXk5hHu').should.equal(true);

    URI.isValid('rhombus:').should.equal(false);
    URI.isValid('rhombus:badUri').should.equal(false);
    URI.isValid('rhombus:pYtcQVUyUSjeE8Htudr3CJsx1FHQMyXHzM?amount=bad').should.equal(false);
    URI.isValid('rhombus:pYtcQVUyUSjeE8Htudr3CJsx1FHQMyXHzM?amount=1.2&req-other=param')
                .should.equal(false);
    URI.isValid('rhombus:?r=https%3A%2F%2Ftest.bitpay.com%2Fi%2F6DKgf8cnJC388irbXk5hHu')
                .should.equal(false);
  });
  ...
  it('do not need new keyword', function() {
    var uri = URI('rhombus:PYHrwJA7DimhfYtzVKdZvExJkxJTHFVPok');
  ...
  describe('instantiation from bitcoin uri', function() {
    ...
    it('parses address', function() {
      uri = new URI('rhombus:PYHrwJA7DimhfYtzVKdZvExJkxJTHFVPok');
      uri.address.should.be.instanceof(bitcore.Address);
      uri.network.should.equal(Networks.livenet);
    });

    it('parses amount', function() {
      uri = URI.fromString('rhombus:PYHrwJA7DimhfYtzVKdZvExJkxJTHFVPok?amount=123.22');
    ...
    it('parses a testnet address', function() {
      uri = new URI('rhombus:paFS5X4XuSY9S2dWSe8XKAhPjShHm47yrb');
      uri.address.should.be.instanceof(bitcore.Address);
      uri.network.should.equal(Networks.testnet);
    });

    it('stores unknown parameters as "extras"', function() {
      uri = new URI('rhombus:PYHrwJA7DimhfYtzVKdZvExJkxJTHFVPok?amount=1.2&other=param');
      uri.address.should.be.instanceof(bitcore.Address);
    ...
    it('throws error when a required feature is not supported', function() {
      (function() {
        return new URI('rhombus:PYHrwJA7DimhfYtzVKdZvExJkxJTHFVPok?amount=1.2&other=param&req-required=param');
      }).should.throw(Error);
    });
  ...
  it('should support double slash scheme', function() {
    var uri = new URI('rhombus://PYHrwJA7DimhfYtzVKdZvExJkxJTHFVPok');
    uri.address.toString().should.equal('PYHrwJA7DimhfYtzVKdZvExJkxJTHFVPok');
  });

  it('should input/output String', function() {
    var str = 'rhombus:PYHrwJA7DimhfYtzVKdZvExJkxJTHFVPok?' +
  ...
  it('should support numeric amounts', function() {
    var uri = new URI('rhombus:PYHrwJA7DimhfYtzVKdZvExJkxJTHFVPok?amount=12.10001');
  ...
  it('should generate a valid URI', function() {
    new URI({
      ...
      'rhombus:PYHrwJA7DimhfYtzVKdZvExJkxJTHFVPok'
    );
    ...
    new URI({
      ...
      'rhombus:PYHrwJA7DimhfYtzVKdZvExJkxJTHFVPok?amount=1.10001&message=Hello%20World&something=else'
    );
  ...
  it('should be case insensitive to protocol', function() {
    ...
    var uri2 = new URI('rhombus:PYHrwJA7DimhfYtzVKdZvExJkxJTHFVPok');

    uri1.address.toString().should.equal(uri2.address.toString());
  });

  it('writes correctly the "r" parameter on string serialization', function() {
    var originalString = 'rhombus:pkMUkHYKJYTZw96SE1VJZCHiRJoWwS5NtJ?amount=0.1&' +
  ...
  it('displays nicely on the console (#inspect)', function() {
    var uri = 'rhombus:PYHrwJA7DimhfYtzVKdZvExJkxJTHFVPok';
