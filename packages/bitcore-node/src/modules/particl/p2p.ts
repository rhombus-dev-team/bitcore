...
export class RhombusP2PWorker extends BaseP2PWorker<IRhomBlock> {
  ...
  constructor({ chain, network, chainConfig, blockModel = RhombusBlockStorage }) {
    ...
    this.invCache = {};
    this.invCacheLimits = {
      [this.bitcoreP2p.Inventory.TYPE.WITNESS_BLOCK]: 100,
      [this.bitcoreP2p.Inventory.TYPE.WITNESS_TX]: 100000
    };
  ...
  setupListeners() {
    ...
    this.pool.on('peerblock', async (peer, message) => {
      ...
      const blockInCache = this.isCachedInv(this.bitcoreP2p.Inventory.TYPE.WITNESS_BLOCK, hash);
      if (!blockInCache) {
        this.cacheInv(this.bitcoreP2p.Inventory.TYPE.WITNESS_BLOCK, hash);
      }
      ...
      if (this.isSyncingNode) {
        const filtered = message.inventory.filter(inv => {
          const hash = this.bitcoreLib.encoding
            ...
            .toString('hex');
          // HAX?
          if (inv.type === this.bitcoreP2p.Inventory.TYPE.BLOCK) {
            inv.type = this.bitcoreP2p.Inventory.TYPE.WITNESS_BLOCK;
          }
          if (inv.type === this.bitcoreP2p.Inventory.TYPE.TX) {
            inv.type = this.bitcoreP2p.Inventory.TYPE.WITNESS_TX;
          }
